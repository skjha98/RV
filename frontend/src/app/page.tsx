...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormData({ flat_number: '', owner_name: '', type: 'FLAT', owner_type: 'OWNER', mobile: '', email: '', name: '', date: '' });
    setShowForm(false);
    fetchData();
  };
        ...
        {showForm && (
          <div className="mb-10 bg-white rounded-3xl border border-slate-200 shadow-2xl shadow-slate-200/50 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50">
              <h2 className="text-lg font-bold text-slate-800 tracking-tight">Create {activeTable.slice(0, -1)}</h2>
            </div>
            <form onSubmit={handleSubmit} className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {activeTable === 'festivals' && (
                <>
                  <div className="col-span-1"><label className={labelClass}>Festival Name</label><input className={inputClass} placeholder="Occasion" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} required /></div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-widest">Reference</th>
                <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-widest">Entity Details</th>
